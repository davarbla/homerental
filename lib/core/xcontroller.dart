...
class XController extends GetxController {
  ...
  AllProvider _provider = AllProvider();
      ...
      _provider.pushResponse('api', jsonBody)!.then((Response? response) {
        if (response != null && response.statusCode == 200) {
          ...
          if (_result['result'] != null && _result['result'].length > 0) {
            ...
            if (mytrans != null && mytrans.length > 0) {
              mytrans.forEach((e) {
            ...
            if (nearbys != null && nearbys.length > 0) {
              nearbys.forEach((e) {
  ...
  getRentReviewById(final String ir, final String iu) {
    try {
      ...
      final jsonBody =
          jsonEncode({"iu": "$iu", "lat": "$latitude", "ir": "$ir"});
      logPrint(jsonBody);
  ...
  getRentByCategory(final String ic) {
    try {
      ...
      final jsonBody =
          jsonEncode({"iu": "$iu", "lat": "$latitude", "ic": "$ic"});
      //logPrint(jsonBody);
    ...
    try {
      ...
      if (response != null && response.statusCode == 200) {
        ...
        if (_result['code'] == '200') {
          ...
          if (updatedRent != null && updatedRent.length > 0) {
  ...
  //member
  final member = Map<String, dynamic>().obs;
  ...
  final install = Map<String, dynamic>().obs;
  ...
  ChatController _chatController = ChatController.instance;
  ...
  updateChattingWith(String uid, String peerId) {
    ...
    chatController.firestore
        .collection(ChatController.TAG_USERCHAT)
        .doc(uid)
        .update({
      'chattingWith': peerId,
